...
#define MMAP_TESTCASE_SIZE 1000*1000*1000
#define MMAP_CRC_SIZE SIZE
...
int main(int argc, char **argv)
{
	...
	fd = open("/dev/zero",
			...
			O_RDWR,
			0);
	mdst = mmap(NULL, MMAP_TESTCASE_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
	if (mdst == MAP_FAILED) {
		perror("mmap dst");
		exit(-1);
	}
...
#endif
#ifdef MMAP
	long long remain_size = MMAP_TESTCASE_SIZE;
...
//		printf("remain_size=%d mdst=%p off=%d n=%d\n",remain_size,mdst,off,n);
		crc = crc32c(crc,mdst + off,n);
		off += n;
	}
	munmap(mdst,MMAP_TESTCASE_SIZE);
	close(fd);
	got = 1;
#endif
